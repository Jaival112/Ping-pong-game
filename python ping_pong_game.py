...
screen_1.title("Jaival's Ping pong game")
...
while True:
    ...
    if (hit_ball.xcor() > right_paddle.xcor()-60) and (hit_ball.ycor() < right_paddle.ycor() + 60 and
                                    hit_ball.ycor() > right_paddle.ycor() - 60):

        hit_ball.setx( right_paddle.xcor()-60)
        hit_ball.dx *= -1

    if (hit_ball.xcor() < left_paddle.xcor() +60) and (hit_ball.ycor() < left_paddle.ycor() + 60 and
                                     hit_ball.ycor() > left_paddle.ycor() - 60):
        hit_ball.setx(left_paddle.xcor()+60)
        hit_ball.dx *= -1
